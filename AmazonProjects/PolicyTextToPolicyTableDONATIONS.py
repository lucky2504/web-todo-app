...
def create_policy_end_to_end_line(file_path):
    ...
    for rule in rules:
        ...
        donationEligible = ''
        outputReasonAttributes = ''

        attributes = []  # Store attributes in a list

        for line in lines[1:]:
            line = line.strip()
            if 'donationEligible |' in line:
                donationEligible = line.split('|')[0].strip()[:-16] + ": " + line.split('|')[1].strip()
            elif 'outputReasonAttributes |' in line:
                outputReasonAttributes = line.split('|')[0].strip()[:-22] + ": " + line.split('|')[1].strip()
        ...
        rule_dict = {
            ...
            'outputReasonAttributes': outputReasonAttributes,
...
def create_policy_attribute_df(policy_data, attributes):
    ...
    base_columns = ['Rules', 'donationEligible', 'outputReasonAttributes', 'Line Count', 'Policy Text', 'Attr 0'] # Initialize new dataframe with base columns
