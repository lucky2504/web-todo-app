...
def create_policy_end_to_end_line(file_path):
    ...
    for rule in rules:
        ...
        for line in lines[1:]:
            line = line.strip()
            if 'wasteCategory |' in line:
                wasteCategory = line.split('|')[0].strip()[:13] + line.split('|')[1].strip()
            if 'wasteCategoryType |' in line:
                wasteCategoryType = line.split('|')[0].strip()[:17] + line.split('|')[1].strip()
            if 'wasteType |' in line:
                wasteType = line.split('|')[0].strip()[:9] + line.split('|')[1].strip()
            if 'wasteStorageClass |' in line:
                wasteStorageClass = line.split('|')[0].strip()[:17] + line.split('|')[1].strip()
        ...
        rule_dict = {
            ...
            'wasteType': wasteType,
            'wasteStorageClass': wasteStorageClass,
...
def create_policy_attribute_df(policy_data, attributes):
    ...
    base_columns = ['Rules', 'wasteCategory', 'wasteCategoryType', 'wasteType', 'wasteStorageClass', 'Line Count', 'Policy Text', 'Attr 0'] # Initialize new dataframe with base columns
