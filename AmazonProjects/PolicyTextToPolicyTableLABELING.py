...
def create_policy_end_to_end_line(file_path):
    ...
    for rule in rules:
        ...
        for line in lines[1:]:
            line = line.strip()
            if 'palletLabels |' in line:
                palletLabels = line.split('|')[0].strip()[:-12] + ": " + line.split('|')[1].strip()
            elif 'packLabels |' in line:
                packLabels = line.split('|')[0].strip()[:-10] + ": " + line.split('|')[1].strip()
            elif 'secondaryLabels |' in line:
                secondaryLabels = line.split('|')[0].strip()[:-15] + ": " + line.split('|')[1].strip()
            elif 'unidStatements |' in line:
                unidStatements = line.split('|')[0].strip()[:-14] + ": " + line.split('|')[1].strip()
            elif 'batteryStatements |' in line:
                batteryStatements = line.split('|')[0].strip()[:-17] + ": " + line.split('|')[1].strip()
            elif 'hazmatLabelStatementsIdentifier |' in line:
                hazmatLabelStatementsIdentifier = line.split('|')[0].strip()[:-31] + ": " + line.split('|')[1].strip()
        ...
        rule_dict = {
            ...
            'unidStatements': unidStatements,
            'batteryStatements': batteryStatements,
            'hazmatLabelStatementsIdentifier': hazmatLabelStatementsIdentifier,
...
def create_policy_attribute_df(policy_data, attributes):
    ...
    base_columns = ['Rules', 'palletLabels', 'packLabels', 'secondaryLabels', 'unidStatements', 'batteryStatements', 'hazmatLabelStatementsIdentifier', 'Line Count', 'Policy Text', 'Attr 0'] # Initialize new dataframe with base columns
