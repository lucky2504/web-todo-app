...
def create_policy_end_to_end_line(file_path):
    ...
    for rule in rules:
        ...
        for line in lines[1:]:
            line = line.strip()
            if 'dropZone |' in line:
                dropZone = line.split('|')[0].strip()[:-8] + ": " + line.split('|')[1].strip()
            elif 'storageLevel |' in line:
                storageLevel = line.split('|')[0].strip()[:-12] + ": " + line.split('|')[1].strip()
            elif 'storageClass |' in line:
                storageClass = line.split('|')[0].strip()[:-12] + ": " + line.split('|')[1].strip()
...
def create_policy_attribute_df(policy_data, attributes):
    ...
    base_columns = ['Rules', 'dropZone', 'storageLevel', 'storageClass',  'Line Count', 'Policy Text', 'Attr 0'] # Initialize new dataframe with base columns
